...
def deleteFixup(RBTree, RBNode):
    while RBNode.parent and not isRed(RBNode):
        if RBNode is RBNode.parent.leftnode:
            ...
            if w and not (isRed(w.leftnode) or isRed(w.rightnode)): # Case 2 \\ Verify if is necessary the first evaluation.
                w.red = True
                RBNode = RBNode.parent
            else:
                if w and not isRed(w.rightnode):
                    w.leftnode.red = False
                    w.red = True
                    rotateRight(RBTree, w)
                    w = RBNode.parent.rightnode
                w.red = isRed(RBNode.parent)
...
for i in range(0, 100):
    if delete(miniTree, random.randint(0,200)):
        count +=1
